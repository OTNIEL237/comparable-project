...
/**
 * Met en place les écouteurs pour tous les formulaires.
 * Centraliser ici évite les erreurs `Cannot read properties of null`.
 */
function setupFormListeners() {
    ...
    document.getElementById('formTheme')?.addEventListener('submit', (e) => { e.preventDefault(); enregistrerTheme(); });
    document.getElementById('formAttribuerTheme')?.addEventListener('submit', (e) => { e.preventDefault(); confirmerAttributionTheme(); });
...
// ===================================================================
// ==          NOUVELLES FONCTIONS POUR GESTION STAGIAIRES          ==
// ===================================================================
...
async function ouvrirModalAttribuerAStagiaire(stagiaireId, stagiaireNom) {
    ...
    const formData = new FormData();
    formData.append('action', 'get_stagiaire_details'); // On réutilise l'endpoint
    formData.append('stagiaire_id', stagiaireId);
...
async function confirmerAttributionThemePourStagiaire(form) {
    ...
    formData.append('action', 'attribuer_theme');
    
    const submitBtn = form.querySelector('button[type="submit"]');
    const originalText = submitBtn.innerHTML;
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Attribution...';
    ...
    try {
        ...
        if (result.success) {
            afficherNotification('Thème attribué avec succès !', 'success');
            fermerModal('modalAttribuerThemeAStagiaire');
            setTimeout(() => window.location.reload(), 1200);
        } else {
            afficherNotification("Erreur lors de l'attribution.", "error");
        }
    } catch (error) {
        afficherNotification("Erreur technique.", "error");
    } finally {
...
// --- AUTRES FONCTIONS (MESSAGERIE, RAPPORTS, TÂCHES, etc.) ---
// ... (Toutes vos autres fonctions JS comme envoyerMessage, validerRapport, etc. restent ici)
// La seule chose importante est que les "addEventListener" sont maintenant dans setupFormListeners()
...
// ... (le reste de vos fonctions comme validerRapport, enregistrerTache, etc.)
